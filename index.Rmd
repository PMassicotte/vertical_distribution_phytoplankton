...
read_csv(here::here("data", "clean", "ctd.csv")) %>%
  filter(owd == 2 & depth_m == 1.98) %>%
  select(station, depth_m, owd) %>%
...
-   `bbp/cp` is often considered as a proxy of particle size and composition.
    
    -   `bbp/cp` has been found to be consistent across a wide range of diverse open ocean environments.

-   `cp` is, in theory, most sensitive to particles in the size range of the majority of phytoplankton (0.5-20 um) while smaller particles are thought to dominate the `bbp` signal.

    -   Note that the smallest particle size measured by the UVP during the Greenedge campaign is 64 um (0.064 mm).

-   `cp` is proportional to the concentration of suspended particles in a size domain overlapping that of the phytoplankton assemblage.

-   `cp` is largely insensitive to changes in intracellular chlorophyll concentration, which varies with growth irradiance (photoacclimation).

-   `cp/chla` (`cp*`) represents an interesting optical measurement to monitor phytoplankton photoacclimation/physiology.

    -   Some studies have reported relationships with E~k~.
...
This graph will be used to show an overview of the physical dynamics in the Baffin Bay. Data from @Randelhoff2019.
...
-   Contrary to what can observed in @Hill2013 (see Fig. 8), we can see *underice* sub-surface chla maximum in the Greenedge data.
...
- Depth of maximum values for chla, Cp and bbp.
- Only showing proxies measured at high vertical resolution. It is unreliable to calculate SM for POC because there are not enought data points.

This figure shows that all SMs are located between the nitracline and the isolume where there are optimal conditions to sustain primary production.

```{r}
knitr::include_graphics(here("graphs", "png", "fig04.png"))
```

This kind of graphic could be interesting to understand the co-variability between max-depth layers.

```{r}
knitr::include_graphics(here("graphs", "png", "fig04b.png"))
```
...
-   NO~3~ was integrated between the surface and h~BD~.
-   Other values (B-C-D) have been integrated between the surface and 0.1 isolume.
...
- For each OWD, I have calculated the averaged depth where the FV/Fm ratio was between 0.4 and 0.75 (often considered the optimal range) in the absence of cyanobacteria. Below, algal growth is considered to be limited by nutrient availability and/or light stress.

- The Fv/Fm ratio follows the SMs and therefore suggest that, contrary to the claim of some studies, this phytoplankton biomass is not just a result of accumulation, but rather active and also able to produce new biomass.
...
```{r}
knitr::include_graphics(here("graphs", "png", "appendix04.png"))
```

### Appendix 5

- Z~0.1~ calculated from the bio-argo data.

- The vertical dashed lines correspond to the averaged Z~0.1~ isolume.

```{r}
knitr::include_graphics(here("graphs", "png", "appendix05.png"))
```
