...
-   bbp/cp ratio (need to correct for CP first)
...
Co-variability of fluorescence of chla, CP and bbp(620). At each OWD, the depth of the maximum value (ex.: chla) was extracted. Note that the depths at which each variable is maximum was calculated on *raw* data and not the interpolated data. The points are the raw data and the lines represent values fitted using loess.
...
-   At low bbp/cp (i.e. higher contribution of small particle to the total pool):

    -   There are more particles.
    -   Chlorophyll-a concentration is higher.
    -   There are more particles in open water than in ice-covered stations.
...
Here I wanted to predict the bloom state:

  - pre-bloom: -30, -10 OWD
  - bloom: -10, 10 OWD
  - post-bloom: 10, 40 OWD

So, I rand a random forest (classification tree) model to predict/classify these three bloom states. The model was validated using monte-carlo simulations performed on training/testing data to avoid overfitting. The next graphic shows the importance of each variable that were included in the model. For instance, the total number of particle (UVP), phaeophytin_a and POC are the three most important variable to classify the bloom state.

```{r, fig.align='center'}
knitr::include_graphics(here("graphs", "png", "fig10.png"))
```

## Appendix

### Appendix 1 (MVP fluorescence)
...
Relationship between Cp and bbp.
