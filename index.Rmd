...
```{r}
knitr::include_graphics(here("graphs", "png", "fig02c.png"))
```
...
-   Panel B shows the determination coefficients of the regression between aphy and chla between 400 and 700 nm.
...
-   0.5-20 μm (0.0005 - 0.02 mm): most of the size domain occupied by marine phytoplankton.
...
-   **Using all the data** (not only \<= 100 meters).
    ...
    -   I used a threshold of 5 meters to discard ctd, uvp and hydroscat measurements that were too far apart.
...
-   We can see that high PP occurred before OWD = 0.
    ...
    -   PP is almost the same in open/ice covered water, but the chla is much high in open water (see panel **B**).

-   The correlation between PP and Chl-a is much higher for ice-covered stations compared to open water stations.

    -   Any ideas why in situ PP is not correlated to the phytoplankton biomass for stations located in open water? This is in contradiction with what is shown by @Hill2013.
...
This is the daily integrated PP (surface to isolume).

```{r}
knitr::include_graphics(here("graphs", "png", "appendix04b.png"))
```
...
-   Determine if CP correction (baseline shift) should be performed or not.
    ...
    -   See script `33_visualize_effect_baseline_correction_cp.R`.
...
-   Check for packaging effect and correlation with aphi443 vs 670?

-   If confidence intervals needed on the estimates, check `37_cp_or_bbp_to_predict_chla.R`.

-   Autocorrelation (see `ccf` function).
